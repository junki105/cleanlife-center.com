...
SQL;
	...
	public function doReset()
	{
		...
		} elseif($wpacuResetValue === 'reset_critical_css') {
			// [CRITICAL CSS]
...
SQL;
	...
	public function resetDone()
	{
		...
		} elseif ($this->resetChoice === 'reset_critical_css') {
		    // [CRITICAL CSS]
	...
	public function importDone()
    {
	    ...
	    foreach ($this->data['import_done_list'] as $importedKey) {
            if ($importedKey === 'settings') {
                ...
                // [wpacu_pro]
	            $importedMessage .= '<li>'.__('Any CSS/JS updated positions (to &lt;HEAD&gt; or &lt;BODY&gt;)', 'wp-asset-clean-up').'</li>';
	            $importedMessage .= '<li>'.__('Any CSS/JS preloading (rel="preload")', 'wp-asset-clean-up').'</li>';
	            $importedMessage .= '<li>'.__('Any defer/async attributes added to the JS files', 'wp-asset-clean-up').'</li>';
	            // [/wpacu_pro]
            ...
            } elseif ($importedKey === 'critical_css_options') {
	            // [CRITICAL CSS]
	            $importedMessage .= '<li>'.__('Critical CSS', 'wp-asset-clean-up').'</li>';
	            // [/CRITICAL CSS]
            }
        }
