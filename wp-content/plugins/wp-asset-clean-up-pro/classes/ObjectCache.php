...
class ObjectCache {
	 ...
	 */
		 ...
		 * @param $data
		 */
		public static function wpacu_cache_add_to_array($key, $data) {
			if ($allData = self::wpacu_cache_get($key)) {
				$allData[] = $data; // append the value to the list
			} else {
				$allData = array($data); // first time when a value is added to the list
			}

			$allData = array_unique($allData);

			self::wpacu_cache_delete( $key );
			self::wpacu_cache_add( $key, $allData );
		}
		// [/wpacu_pro]
