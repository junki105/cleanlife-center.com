...
SQL;
	     ...
	     */
	    $loadExceptionsClass = new LoadExceptions();
	    ...
	    $allPossibleDataTypes = array('load_it_logged_in', 'preloads', 'positions', 'media_queries_load', 'notes', 'ignore_child', 'everywhere', 'date', '404', 'search');
	...
	public function pageOverview()
	{
		...
		// Any plugins unloaded site-wide (with exceptions) or based on other conditions? Get all rules for both active & inactive plugins
		...
		if ( ! empty($getAllPluginsRules) ) {
            ...
            foreach ($allPlugins as $pluginPath => $pluginData) {
                ...
                foreach (array_keys($getAllPluginsRules) as $locationKey) {
	                if ( ! isset( $getAllPluginsRules[ $locationKey ][ $pluginPath ] ) ) {
		                continue;
	                }

	                $pluginsWithRules[$locationKey][] = array(
		                'title' => $pluginData['Name'],
		                'path'  => $pluginPath,
		                'rules' => $getAllPluginsRules[ $locationKey][ $pluginPath ]
	                );
                }
            }

			foreach (array_keys($getAllPluginsRules) as $locationKey) {
				usort( $pluginsWithRules[$locationKey], static function( $a, $b ) {
			...
			$this->data['plugins_active']     = get_option('active_plugins', array());
			$this->data['plugins_with_rules'] = $pluginsWithRules; // all rules for all plugins
	...
	public static function renderHandleTd($handle, $assetType, $data, $for = 'default')
	{
        ...
        if ( $for === 'default' ) {
	        ...
	        if (isset($handleData['media_queries_load']['enable'], $handleData['media_queries_load']['value'])
		        ...
		        ?>
                <div><small><span class="dashicons dashicons-desktop" style="vertical-align: middle;"></span> Downloads if the media query matches: <code><?php echo ucfirst(htmlspecialchars($data['handles'][$assetType][$handle]['media_queries_load']['value'])); ?></code></small></div>
		        <?php
	        }
	        // [/wpacu_pro]
