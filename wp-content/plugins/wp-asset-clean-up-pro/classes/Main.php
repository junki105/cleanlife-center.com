...
SQL;
    ...
    public function printScriptsStyles()
    {
        ...
        */
        ...
        if ($isFrontEndEditView) {
	        ...
	        // [wpacu_pro]
	        ObjectCache::wpacu_cache_set('wpacu_data_current_unloaded', $data['current']);
			// [/wpacu_pro]
    ...
    public function ajaxGetJsonListCallback()
    {
        ...
        if (isset($data['all']['unloaded_plugins']) && ! empty($data['all']['unloaded_plugins'])) {
            $GLOBALS['wpacu_filtered_plugins'] = $data['all']['unloaded_plugins'];
        }
        // [/wpacu_pro]
	    ...
	    if ($postId > 0) {
		    ...
		    if (isset($postData->post_type) && $postData->post_type) {
			    ...
			    $data['is_bulk_unloadable']         = true;
			    // [wpacu_pro]
			    ObjectCache::wpacu_cache_set('wpacu_data_bulk_unloaded', $data['bulk_unloaded']);
			    // [/wpacu_pro]
		    }
	    }
        ...
        // [wpacu_pro]
	    ObjectCache::wpacu_cache_set('wpacu_data_load_exceptions', $data['load_exceptions']);
		// [/wpacu_pro]
	    ...
	    ObjectCache::wpacu_cache_set('wpacu_data_unloads_regex_matches', $data['unloads_regex_matches']);

	    $data['load_exceptions_regex_matches'] = array(
		    'styles'  => isset($this->loadExceptionsRegEx['current_url_matches']['styles'])  ? $this->loadExceptionsRegEx['current_url_matches']['styles'] : array(),
		    'scripts' => isset($this->loadExceptionsRegEx['current_url_matches']['scripts']) ? $this->loadExceptionsRegEx['current_url_matches']['scripts'] : array()
	    );
	    ObjectCache::wpacu_cache_set('wpacu_data_load_exceptions_regex_matches', $data['load_exceptions_regex_matches']);

	    // Get all rules from the database
	    $data['handle_unload_regex'] = $this->unloadsRegEx;
	    $data['handle_load_regex']   = $this->loadExceptionsRegEx;
        ObjectCache::wpacu_cache_set('wpacu_data_handle_unload_regex', $data['handle_unload_regex']);
        ObjectCache::wpacu_cache_set('wpacu_data_handle_load_regex', $data['handle_load_regex']);
	    ...
	    // [wpacu_pro]
	    ObjectCache::wpacu_cache_set('wpacu_data_handle_load_logged_in', $data['handle_load_logged_in']);
		// [/wpacu_pro]

	    $data['handle_notes'] = $this->getHandleNotes();

	    // [wpacu_pro]
	    ObjectCache::wpacu_cache_set('wpacu_data_handle_notes', $data['handle_notes']);
		// [/wpacu_pro]
