...
<div class="wpacu-handle-notes">
	<?php if (! $handleNote) { ?>
		<p><small>No notes have been added about this hardcoded <?php echo $tagType; ?> tag (e.g. why you unloaded it or decided to keep it loaded) &#10230; <a data-handle="<?php echo $data['row']['obj']->handle; ?>" href="#" class="wpacu-add-handle-note wpacu-for-style"><span class="dashicons dashicons-welcome-write-blog"></span> <label for="wpacu_handle_note_<?php echo $data['row']['obj']->handle; ?>">Add Note</label></a></small></p>
	<?php } else { ?>
		<p><small>The following note has been added for this hardcoded <?php echo $tagType; ?> tag (<em>to have it removed on update, just leave the text area empty</em>):</small></p>
	<?php } ?>
	<div <?php if ($handleNote) { echo 'style="display: block;"'; } ?>
		...
		class="wpacu-handle-notes-field">
                <textarea id="wpacu_handle_note_style_<?php echo $data['row']['obj']->handle; ?>"
                          ...
                          placeholder="<?php echo sprintf(esc_attr__('Add your note here about this hardcoded %s tag', 'wp-asset-clean-up'), $tagType); ?>"
