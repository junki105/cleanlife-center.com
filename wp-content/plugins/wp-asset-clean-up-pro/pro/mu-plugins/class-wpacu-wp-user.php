...
/**
 * Core class used to implement the WPACU_WP_User object.
 *
 * @since 2.0.0
 *
...
class WPACU_WP_User {
	 ...
	 * Constructor.
	 *
	 * Retrieves the userdata and passes it to WPACU_WP_User::init().
	 *
	 * @since 2.0.0
	 *
	 * @param int|string|stdClass|WPACU_WP_User $id User's ID, a WPACU_WP_User object, or a user object from the DB.
	...
	public function __construct( $id = 0, $name = '', $site_id = '' ) {
		...
		if ( $id instanceof WPACU_WP_User ) {
	...
	public function __isset( $key ) {
		if ( 'id' == $key ) {
			_deprecated_argument(
				...
				'2.1.0',
				sprintf(
				/* translators: %s: WPACU_WP_User->ID */
					__( 'Use %s instead.' ),
					'<code>WPACU_WP_User->ID</code>'
				)
			);
			$key = 'ID';
		}
	 ...
	 * the value on the WPACU_WP_User instance.
	 *
	 * @since 3.3.0
	 *
	 ...
	 *
	 * @since 2.1.0
	 * @deprecated 4.9.0 Use WPACU_WP_User::for_site()
	 *
	 * @global wpdb $wpdb WordPress database abstraction object.
	 *
	...
	protected function _init_caps( $cap_key = '' ) {
		...
		_deprecated_function( __METHOD__, '4.9.0', 'WPACU_WP_User::for_site()' );
	...
	public function has_cap( $cap, ...$args ) {
		...
		$caps = wpacu_map_meta_cap( $cap, $this->ID, ...$args );
		 ...
		 * }
		 * @param WPACU_WP_User  $user    The user object.
		 */
		$capabilities = apply_filters( 'wpacu_user_has_cap', $this->allcaps, $caps, $args, $this );
