( function ( _, $, tbRemover, ajaxurl, settingsEncryptionVariables )
{
    ...
    function responseNotice ( message, type, parent )
    {
        ...
        parent.insertAdjacentHTML(
            'beforebegin',
            '<div id="bwu_encrypt_notice" class="notice notice-' + type + '"><p>' + message + '</p></div>'
        );
    }
    ...
    var SettingEncryption = {
        ...
        generateSymmetricKey: function ( evt )
        {
            ...
            onDone = function ( response )
            {
                ...
                this.symmetricKeyDownloader.setAttribute(
                    'href',
                    'data:application/octet-stream;charset=utf-16le;base64,' + btoa( data.key )
                );
            ...
            onDone = function ( response )
            {
                ...
                if ( !response.success ) {
                    responseNotice(
                        data.message,
                        'error',
                        this.asymmetricModal
                    );
                    return;
                }

                publicKeyDownloader = this.generatedKeyContainer.querySelector( '#asymmetric_generated_public_key_downloader' );
                privateKeyDownloader = this.generatedKeyContainer.querySelector( '#asymmetric_generated_private_key_downloader' );

                if ( !publicKeyDownloader || !privateKeyDownloader ) {
                    return;
                }

                this.generatedKeyContainer.querySelector( '.bwu-generated-key__public .bwu-the-key' ).innerText = publicKey;
                this.generatedKeyContainer.querySelector( '.bwu-generated-key__private .bwu-the-key' ).innerText = privateKey;

                publicKeyDownloader.setAttribute( 'href', 'data:text/plain;base64,' + btoa( publicKey ) );
                privateKeyDownloader.setAttribute( 'href', 'data:text/plain;base64,' + btoa( privateKey ) );
        ...
        {
            var publicKey = this.generatedKeyContainer.querySelector( '.bwu-generated-key__public .bwu-the-key' ).innerHTML;
        ...
        construct: function ()
        {
            ...
            this.symmetricKeyDownloader = this.tab.querySelector( '#symmetric_key_downloader' );
            this.asymmetricPublicKey = this.tab.querySelector( '#asymmetric_public_key_code' );
            this.asymmetricPublicKeyField = this.tab.querySelector( '#asymmetric_public_key' );
            this.symmetricKeyGenerator = this.tab.querySelector( '#symmetric_key_generator' );
            this.asymmetricKeyGenerator = this.tab.querySelector( '#asymmetric_key_pair_generator' );
            this.asymmetricKeyOpenValidateModal = this.tab.querySelector( '#asymmetric_key_open_validate_modal' );

            this.asymmetricModal = document.querySelector( '#asymmetric_generated_key_modal' );
            this.generatedKeyContainer = this.asymmetricModal.querySelector( '.bwu-generated-key' );
            this.privateKeyDownloader = this.asymmetricModal.querySelector( '#asymmetric_generated_private_key_downloader' );
            this.asymmetricKeySelector = this.asymmetricModal.querySelector( '#asymmetric_keys_selector' );
            this.keyHasBeenDownloaded = true;

            this.asymmetricValidateModal = document.querySelector( '#asymmetric_key_pair_validate' );
            this.asymmetricKeyDoValidation = this.asymmetricValidateModal.querySelector( '#asymmetric_key_pair_do_validation' );
            this.asymmetricPrivateKeyField = this.asymmetricValidateModal.querySelector( '#private_key_validate_area' );
        ...
        {
            _.each( this.encryptionKeyOptions, function ( el )
            {
                el.addEventListener( 'change', this.toggleEncryptionType );
            }.bind( this ) );

            this.symmetricKeyGenerator.addEventListener( 'click', this.generateSymmetricKey );
            this.asymmetricKeyGenerator.addEventListener( 'click', this.generateAsymmetricKey );

            this.symmetricKeyDownloader.addEventListener( 'click', this.enableSaveSettings );
            this.privateKeyDownloader.addEventListener( 'click', this.enableSaveSettings );
            this.asymmetricKeySelector.addEventListener( 'click', this.asymmetricPublicKeyValueToField );
            this.asymmetricKeyOpenValidateModal.addEventListener( 'click', this.validateAsymmetricKeysModal );
            this.asymmetricKeyDoValidation.addEventListener( 'click', this.validateAsymmetricKey );
